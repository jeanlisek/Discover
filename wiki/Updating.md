...
How to update Discover to a newer version without losing data.
...
If you started Discover with `docker run`, pull the new image and replace the container:
...
If you installed Discover via the [Proxmox VE Community Scripts](https://community-scripts.org/scripts/trek), run the following command inside the **LXC container** and select **Update** when prompted:
...
Open the **Stacks** list, click the Discover stack, then click **Redeploy**.
...
In the Unraid Docker tab, click the Discover container and select **Update**. Unraid will pull the latest image and restart with the same volumes.
