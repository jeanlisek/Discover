...
Discover supports four delivery channels. Which channels appear depends on what the admin has enabled server-side.
...
| **Webhook** | Discover POSTs a JSON payload to a URL you specify. Discord and Slack webhook URLs are auto-detected and receive a natively formatted payload. |
...
|-------|-------------|---------|
| `version_available` | A new Discover version is available | in-app, email, webhook, ntfy |
...
Enter a URL that Discover will POST to when a notification fires. Once saved, the URL is displayed as `••••••••`. Use the **Test** button to send a test payload to the saved URL.

Discover auto-detects the webhook destination and adjusts the payload format:
...
- **Generic** — sends a plain JSON object with `event`, `title`, `body`, `tripName`, `link`, `timestamp`, and `source` (`"Discover"`) fields.
