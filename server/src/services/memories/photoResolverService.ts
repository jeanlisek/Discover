...
import type { DiscoverPhoto } from '../../types';
...
import * as photoCache from './discoverPhotoCache';
import { ensureLocalThumbnail } from './thumbnailService';

// ── Lookup / Register ────────────────────────────────────────────────────

export function getOrCreateDiscoverPhoto(
  provider: string,
  assetId: string,
  ownerId: number,
  passphrase?: string,
  ...
  const existing = db.prepare(
    'SELECT id FROM discover_photos WHERE provider = ? AND asset_id = ? AND owner_id = ?'
  ).get(provider, assetId, ownerId) as { id: number } | undefined;
  if (existing) {
    if (passphrase) {
      db.prepare('UPDATE discover_photos SET passphrase = ? WHERE id = ?')
  ...
  const res = db.prepare(
    'INSERT INTO discover_photos (provider, asset_id, owner_id, passphrase) VALUES (?, ?, ?, ?)'
  ).run(provider, assetId, ownerId, passphrase ? encrypt_api_key(passphrase) : null);
  return Number(res.lastInsertRowid);
}

export function getOrCreateLocalDiscoverPhoto(
  filePath: string,
  ...
  const existing = db.prepare(
    "SELECT id FROM discover_photos WHERE provider = 'local' AND file_path = ?"
  ).get(filePath) as { id: number } | undefined;
  if (existing) return existing.id;

  const res = db.prepare(
    'INSERT INTO discover_photos (provider, file_path, thumbnail_path, width, height) VALUES (?, ?, ?, ?, ?)'
  ).run('local', filePath, thumbnailPath || null, width || null, height || null);
  return Number(res.lastInsertRowid);
}

export function resolveDiscoverPhoto(photoId: number): DiscoverPhoto | null {
  return db.prepare('SELECT * FROM discover_photos WHERE id = ?').get(photoId) as DiscoverPhoto | undefined || null;
}
...
export async function streamPhoto(
  ...
  const photo = resolveDiscoverPhoto(photoId);
  if (!photo) {
    ...
    if (kind === 'thumbnail') {
      ...
      if (!thumbRel) {
        ...
        if (result) {
          ...
          db.prepare(
            'UPDATE discover_photos SET thumbnail_path = ?, width = COALESCE(width, ?), height = COALESCE(height, ?) WHERE id = ?'
...
export function setDiscoverPhotoProvider(
  discoverPhotoId: number,
  provider: string,
  assetId: string,
  ownerId: number,
): void {
  db.prepare(
    'UPDATE discover_photos SET provider = ?, asset_id = ?, owner_id = ? WHERE id = ?'
  ).run(provider, assetId, ownerId, discoverPhotoId);
}

// ── Orphan cleanup ───────────────────────────────────────────────────────

export function deleteDiscoverPhotoIfOrphan(photoId: number): void {
  ...
  `).get(photoId, photoId);
  if (stillUsed) return;
  db.prepare("DELETE FROM discover_photos WHERE id = ? AND provider != 'local'").run(photoId);
}

// ── Delete local file for a trek_photo ──────────────────────────────────

export function getDiscoverPhotoFilePath(photoId: number): string | null {
  const photo = resolveDiscoverPhoto(photoId);
