...
export function canAccessUserPhoto(requestingUserId: number, ownerUserId: number, tripId: string, assetId: string, provider: string): boolean {
    ...
    if (tripId === '0') {
        const journeyPhoto = db.prepare(`
            ...
            JOIN discover_photos tkp ON tkp.id = gp.photo_id
            WHERE tkp.asset_id = ?
              AND tkp.provider = ?
              AND tkp.owner_id = ?
            LIMIT 1
    ...
    // Regular trip photos — join through discover_photos
...
// ── Unified photo access check (discover_photos based) ──────────────────────

export function canAccessDiscoverPhoto(requestingUserId: number, discoverPhotoId: number): boolean {
    const photo = db.prepare('SELECT * FROM discover_photos WHERE id = ?').get(discoverPhotoId) as { id: number; provider: string; owner_id: number | null } | undefined;
    ...
    const tripAccess = db.prepare(`
        ...
        LIMIT 1
    `).get(discoverPhotoId, requestingUserId, requestingUserId);
